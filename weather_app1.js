	...
	$.ajax({
		...
		success: function(fore){
			...
			for(var i=0; i< fore.list.length; i++){
				...
				futTemp.push(fore.list[i].main.temp - 273);}
				// var futHumid=fore.list[i].main.humidity;
				// var futDescrip=fore.list[i].weather[0].description;
				// console.log(startDate,futTemp,futHumid,futDescrip);
				// var htmlNewTable='<tr><td>' + startDate + /*'</td><td>' + endDate +*/ '</td><td>' + futTemp + '</td><td>' + futHumid + '</td><td>' + futDescrip + '</td></tr>';
				// $("tbody").append(htmlNewTable);
